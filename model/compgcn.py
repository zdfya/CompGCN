...
class CompGCN(nn.Module):
    ...
    def init_model(self):
        ...
        self.GraphCov = CompGCNCov(
            ...
            act=torch.tanh,
            bias=False,
            drop_rate=self.args.gcn_drop,        # 图卷积 Dropout，可在 args 中设置
            opn=self.args.opn
        )
        ...
        conv_input_dim = 2 * self.args.k_h * self.args.k_w
        self.conv_input_proj = nn.Linear(self.args.emb_dim, conv_input_dim)
    ...
    def _forward_conve(self, sub_emb, rel_emb, all_ent):
        """
        sub_emb / rel_emb: (B, gcn_dim)
        all_ent: (num_ent, gcn_dim)  或者 (num_ent, conv_input_dim)
        """

        # —— 投影到 ConvE 期望维度 —— 
        sub_emb = self.conv_input_proj(sub_emb)  # -> (B, 2*k_h*k_w)
        rel_emb = self.conv_input_proj(rel_emb)  # -> (B, 2*k_h*k_w)

        # 1) reshape + BN -> Conv -> BN -> ReLU -> Drop2d
        B = sub_emb.size(0)
        h, w = 2 * self.args.k_h, self.args.k_w
        x = torch.cat([
            sub_emb.view(B, 1, h, w),
            rel_emb.view(B, 1, h, w)
        ], dim=1)  # (B,2,h,w) 如果 conv1 接收 2 个通道则改成 in_channels=2

        # 如果 conv1 设计成 in_channels=1，先拼在一起再重新做 BN/Conv
        x = self.bn0(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x = F.relu(x)
        ...
        # 3) 与所有实体 embeddings 相乘 -> bias -> sigmoid
        # 注意 all_ent 要么是 gcn_dim 再投影一次，要么直接存成 conv_input_dim

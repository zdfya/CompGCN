    ...
    def fit(self):
        ...
        for epoch in range(1, self.p.max_epochs + 1):
            ...
            if curr_mrr > self.best_val_mrr:
                ...
                logger.info(f"Epoch {epoch}: new best MRR={curr_mrr:.4f}, loss={loss:.4f}, time={elapsed:.1f}s, model saved.")
            else:
                logger.info(f"Epoch {epoch}: MRR={curr_mrr:.4f}, loss={loss:.4f}, time={elapsed:.1f}s")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--node_drop', dest='node_drop', type=float, default=0.1,help='Dropout rate for node features after GCN')
